...
![alt text](https://img.shields.io/badge/license-Elastic%20License%202.0-blue)

![alt text](https://img.shields.io/badge/version-8.x-orange)
...
Beats & Logstash (Ingestion): Data is collected from source systems using Beats (lightweight shippers) or sent directly to Logstash. Logstash then parses, enriches, and transforms the data.
Elasticsearch (Storage & Search): Logstash sends the processed data to Elasticsearch, a distributed search and analytics engine, which indexes and stores the data.
Kibana (Visualization & Management): Kibana connects to Elasticsearch and provides a powerful web UI to explore, visualize, and build dashboards on top of the data.
Core Components
...
WSL 2: The Windows Subsystem for Linux is required for Docker Desktop. Installation Guide.
Docker Desktop for Windows: The easiest way to run the stack. Download here.
Terminal: Windows Terminal or PowerShell 7+.
Software (Linux)
OS: A modern Linux distribution (e.g., Ubuntu 20.04+, CentOS 8+, Debian 10+).
Docker: Latest stable version. Installation Guide.
...
Install Java: Elasticsearch requires a compatible Java Development Kit (JDK). Refer to the Elastic support matrix for the correct version.
Download Binaries: Download the zip (Windows) or tar.gz (Linux) archives for Elasticsearch, Logstash, and Kibana.
Extract Archives: Extract each component into its own directory (e.g., C:\ELK\elasticsearch).
...
Click the menu icon (hamburger) in the top-left, then go to Stack Management > Kibana > Index Patterns.
Click Create index pattern.
Kibana will detect the new index. It should be named logstash-YYYY.MM.DD. Type logstash* into the "Index pattern name" field. It should say "✅ Your index pattern matches 1 source".
Click Next step.
For the "Time field", select @timestamp.
Click Create index pattern.
Step 4: Explore Your Data
Click the menu icon again and go to Analytics > Discover.
You should now see your log entry in the main window! The fields (service, level, message) will be listed on the left.
![alt text](./docs/images/kibana-discover.png)
API Reference
...
Backups: Use Elasticsearch's Snapshot and Restore functionality to back up your data to a remote repository like S3, Azure Blob Storage, or a shared filesystem.
Example: Production Docker Compose
...
# ... (Logstash service would also need username/password)
...
Use code with caution.
Yaml
For true high availability, you would use an orchestration tool like Kubernetes with the Elastic Cloud on Kubernetes (ECK) operator.
Troubleshooting
...
Solution:
Check the Elasticsearch container logs: docker logs elasticsearch. Look for errors.
Wait a few minutes. Elasticsearch can take time to start.
Verify the ELASTICSEARCH_HOSTS environment variable in your docker-compose.yml for the Kibana service is correct (http://elasticsearch:9200).
...
Solution:
Check Logs: docker logs elasticsearch. The error message is key.
Increase Memory (Windows/macOS): Go to Docker Desktop settings > Resources and increase the memory allocation to at least 8GB.
Check vm.max_map_count (Linux): The value must be at least 262144. Run sudo sysctl -w vm.max_map_count=262144. To make it permanent, add vm.max_map_count=262144 to /etc/sysctl.conf.
...
Solution:
Check the Logstash container logs: docker logs logstash. Look for parsing errors or connection issues to Elasticsearch.
Verify your Logstash output configuration points to the correct Elasticsearch host (http://elasticsearch:9200).
In Kibana, ensure you have created an index pattern (logstash*) that matches the index created by Logstash.
Contributing
...
Make your changes and ensure you add or update relevant tests.
...
For more details, see the Elastic License page.
